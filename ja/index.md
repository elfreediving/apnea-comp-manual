...
2. [サインイン](#サインイン)
3. [クイックスタート](#クイックスタート)
4. [役割と各役割でできること](#役割と各役割でできること)
5. [イベントの作成（Organizer）](#イベントの作成organizer)
6. [イベントへの参加（Staff）](#イベントへの参加staff)
7. [選手として参加する](#選手として参加する)
8. [Setup: 選手、Line、OT](#setup-選手lineot)
9. [Break Times](#break-times)
10. [チーム戦（Team）](#チーム戦team)
11. [Check-in](#check-in)
12. [Start List & Speaker Mode](#start-list--speaker-mode)
13. [判定結果の入力](#判定結果の入力)
14. [ペナルティ（Rulebook 17.8）](#ペナルティrulebook-178)
15. [Protests（異議申し立て）](#protests異議申し立て)
16. [Re-swim & Opener](#re-swim--opener)
17. [スケジュール調整（OT Delay）](#スケジュール調整ot-delay)
18. [プッシュ通知](#プッシュ通知)
19. [オフライン動作](#オフライン動作)
20. [AIDA 連携](#aida-連携)
21. [複数日イベント](#複数日イベント)
22. [結果とエクスポート](#結果とエクスポート)
23. [表示 & 言語](#表示--言語)
24. [アカウントの削除](#アカウントの削除)
25. [FAQ](#faq)
26. [トラブルシューティング](#トラブルシューティング)
27. [プライバシーとデータ](#プライバシーとデータ)
28. [連絡先](#連絡先)
...
Apnea Comp では、役割・イベント・結果をデバイスをまたいで保持するためにアカウントが必要です。サインインの方法は 3 通りあります。

- **メールアドレスとパスワード** — アカウントを作成するには **Sign Up** をタップします（メールに届くワンタイムコードで確認します）。すでにアカウントがある場合は **Sign In** をタップします。
- **Continue with Apple** — Apple ID でサインインします。メールアドレスを非公開にした場合、Apple がアプリにプライベートリレーのアドレスを渡すので、すべて問題なく動作します。
- **Continue with Google** — Google アカウントでサインインします。

![Apple と Google のあるサインイン画面](../images/sign-in-social.png)

最初に選んだ方法を、その後もずっと使ってください — 別の方法でサインインすると、別の空のアカウントが作られてしまいます。サインインはデバイスごとに一度だけです。アプリはサインイン状態を保持し、セッションを自動的に復元します。
...
アカウントはアプリ内から削除できます — メールは不要です。

1. **アバター**（上部の隅）をタップ → **Edit Profile**。
2. 一番下までスクロールして **Delete Account** をタップします。
3. ダイアログで確認します。

![Edit Profile 内の Delete Account](../images/delete-account.png)

これにより、あなたのアカウントとプロフィールが完全に削除されます。あなたが所有していない大会データは削除されません — 例えば、選手として参加した場合、organizer のスタートリストや結果に含まれるあなたのエントリーは、その大会の記録の一部として残ります。
