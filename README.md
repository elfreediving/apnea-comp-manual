...
A reliable internet connection (optional during the day — see Offline operation).
...
Open the app, tap + on the Events screen.
Enter event name, type (Pool / Depth / Team), dates, and tap Create.
Open the event → Edit Event → paste AIDA Token and AIDA Event ID → tap Test Connection → Save.
Setup tab → Load Athletes to pull the start list from AIDA.
Setup tab → set Lines (number of lanes), First OT, and Interval between athletes.
Share the event invite code (visible in Edit Event) with your staff.
Approve staff in Users tab as they join (they appear as Pending).

For Staff (judges, check-in officials, …)

Open the app, sign in.
Events screen → tap Join with code → paste the invite code.
You appear as Pending until the Organizer approves you and assigns a role (Judge, Main Judge, Staff, …).

You are now ready to run the day.

Roles and what each can do
RoleSetupJudgeCheck-inUsersLogOT Delay👑 Organizer✅✅✅✅✅✅⚖️ Main Judge✅✅✅✅✅✅✏️ Judge—✅✅———👀 Staff—view✅———⏳ Pending——————
...
Sign in to the app.
On the Events screen, tap Join with code.
Paste the code. The event appears in your list with status Pending.
Wait for the Organizer to approve you and assign a role. You will see the event become tappable once approved.
...
RP — realised performance (time for static, distance for dynamic, depth for depth).
Card — White (clean), Yellow (penalty), or Red (DQ).
Penalty reasons — required when a card other than White is selected. See Penalties.
Start offset — seconds early (negative) or late (positive). Used for the start-window penalty calculation.
REMARKS — required for certain reasons (BO, Other Penalty, DQ Other, etc.). The app pre-fills a template; complete the missing detail.
Tap Save Result.
...
⛔ Offline — saved locally on this device; will sync automatically once you are back online. See Offline operation.
...
Start window
DisciplineWindowWithin windowPast windowPool±10 s1 pt / 5 s (Yellow Early/Late Start)DQ Late Start (Red) for late side; Jump Start (Red, DYN only) for early-pool wall startDepth±30 s1 pt / 5 s (Yellow Early/Late Start)DQ Late Start (Red) for late side
REMARKS
...
Tap 🕒 → the Adjust Schedule dialog opens.
Pick the athlete the delay starts from (the first one whose OT shifts).
Enter the delay in minutes (whole minutes, positive only).
Preview shows the new times — old → new for each affected athlete.
Tap Apply.
...
Offline → ⛔ Offline badge. Auto-retries when online; see Offline operation.
