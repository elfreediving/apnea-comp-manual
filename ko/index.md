...
3. [빠른 시작](#빠른-시작)
4. [역할별 권한](#역할별-권한)
5. [이벤트 생성 (Organizer)](#이벤트-생성-organizer)
6. [이벤트 참여 (Staff)](#이벤트-참여-staff)
7. [선수로 참여하기](#선수로-참여하기)
8. [Setup: 선수, 라인, OT](#setup-선수-라인-ot)
9. [Break Times (휴식 시간)](#break-times-휴식-시간)
10. [팀 대회 (Team)](#팀-대회-team)
11. [체크인](#체크인)
12. [Start List & Speaker Mode](#start-list--speaker-mode)
13. [판정 결과 입력](#판정-결과-입력)
14. [페널티 (룰북 17.8)](#페널티-룰북-178)
15. [이의제기 (Protest)](#이의제기-protest)
16. [재경기 & Opener](#재경기--opener)
17. [일정 조정 (OT Delay)](#일정-조정-ot-delay)
18. [푸시 알림](#푸시-알림)
19. [오프라인 동작](#오프라인-동작)
20. [AIDA 연동](#aida-연동)
21. [멀티데이 이벤트](#멀티데이-이벤트)
22. [결과 및 내보내기](#결과-및-내보내기)
23. [화면 & 언어](#화면--언어)
24. [계정 삭제](#계정-삭제)
25. [FAQ](#faq)
26. [트러블슈팅](#트러블슈팅)
27. [개인정보 및 데이터](#개인정보-및-데이터)
28. [연락처](#연락처)
...
Apnea Comp은 계정이 필요합니다 — 역할, 이벤트, 결과가 여러 디바이스에 걸쳐 사용자와 함께 유지되도록 하기 위해서입니다. 세 가지 방법으로 로그인할 수 있습니다:

- **이메일과 비밀번호** — **Sign Up**을 탭해서 계정을 만들거나 (일회용 코드로 이메일을 확인하게 됩니다), 이미 계정이 있으면 **Sign In**을 탭합니다.
- **Continue with Apple** — Apple ID로 로그인합니다. 이메일을 숨겨도 Apple이 앱에 비공개 릴레이 주소를 제공하므로 모든 기능이 정상 동작합니다.
- **Continue with Google** — Google 계정으로 로그인합니다.

![Apple과 Google이 있는 로그인 화면](../images/sign-in-social.png)

처음에 선택한 방법을 계속 사용하세요 — 다른 방법으로 로그인하면 별개의 빈 계정이 새로 만들어집니다. 디바이스당 한 번만 로그인하면 되며, 앱이 로그인 상태를 유지하고 세션을 자동으로 복원합니다.
...
자동 카운트다운 cue와 별개로, OT 한참 전에 수동으로 한 번 안내를 내보내고 싶을 때가 있습니다 — 예: "다음 선수 OT 10분 전입니다."

Start List의 **Announce** 버튼을 탭하면 OT 몇 분 전을 안내할지 묻는 입력창이 뜹니다. 분을 입력하면 기기의 TTS가 **"N minutes to Official Top"**을 발화합니다.

- 자동 카운트다운 cue와 달리, 이것은 직접 누를 때만 한 번 나가는 **수동 안내**입니다.
- 발화 언어는 기기 언어를 따릅니다 (en / ko / ja / zh / zh_TW).
...
### 백그라운드 음성 (Speaker Mode 한정)

Speaker Mode에서는 음성(카운트다운 cue + 안내)이 앱을 백그라운드로 보내거나 화면을 잠가도 계속 재생됩니다. 즉, 안내용 폰을 잠가두거나 다른 앱을 잠깐 봐도 행사장 스피커로 카운트다운이 끊기지 않습니다.

- **iOS** — 무음 keep-alive 트랙을 유지해 오디오 세션이 살아 있도록 합니다.
- **Android** — 포그라운드 서비스 알림("Start list countdown active")을 띄워 음성이 계속 재생되도록 합니다.

이 백그라운드 재생은 **Speaker Mode에서만** 동작합니다. 일반 Start List 화면에서는 화면이 켜져 있는 동안에만 음성이 재생됩니다.
...
## 판정 계산기 (Judge Calculator)

RP를 손으로 환산하는 대신, Pool / Team 대회에서는 앱의 내장 계산기로 거리와 시간을 산출해 RP 칸에 바로 채울 수 있습니다. 계산기는 **Judge 탭 → 선수 상세 시트**에서 종목에 맞는 아이콘으로 열립니다.

### Pool Length(m) 설정

거리 계산기를 쓰려면 먼저 이벤트의 **Pool Length(m)**를 설정해야 합니다. 이벤트 **Setup**에서 입력하며, **Pool / Team 대회 유형에서만** 표시됩니다 (Depth는 해당 없음). lap 수를 거리로 환산할 때 이 값이 쓰입니다.

### Distance Calc (DYN / DYNB / DNF)

선수 상세 시트의 자(ruler) 버튼을 탭하면 거리 계산기가 열립니다.

1. 완주한 **lap 수**를 입력.
2. 마지막 불완전 lap의 **부분 거리(m)**를 입력.
3. 결과 = lap 수 × Pool Length + 부분 거리.
4. **Fill RP**를 탭하면 산출된 거리가 RP 칸에 자동 입력됩니다.

### STA Average Calc (STA)

선수 상세 시트의 타이머 버튼을 탭하면 평균 시간 계산기가 열립니다.

1. 두 심판의 측정 시간을 각각 `mm:ss`로 입력.
2. 결과 = 두 시간의 평균(가장 가까운 초로 반올림).
3. **Fill RP**를 탭하면 RP 칸에 자동 입력됩니다.

### 참고

- **RP < AP**이면 White 카드가 자동으로 비활성화됩니다 (AP 미달은 White를 줄 수 없음).
- Depth 종목(CWT / CWTB / FIM / CNF)에는 계산기가 없습니다 — RP를 직접 입력하세요.
...
결정을 한 사람이 직접 내리는 대신, jury가 투표로 정할 수 있습니다. Judge와 Main Judge가 각자 **Accept** 또는 **Reject**에 투표합니다.

- 투표는 **익명**입니다 — 누가 무엇에 투표했는지는 공개되지 않고, 양쪽 집계 수만 표시됩니다.
- **동점**이면 Main Judge의 캐스팅(deciding) 투표로 결정됩니다.
- 최종 결정 폼은 **Jury 서명**이 필수입니다.
...
계정은 앱 안에서 직접 삭제할 수 있습니다 — 이메일이 필요 없습니다.

1. **아바타**(상단 모서리) 탭 → **Edit Profile**.
2. 맨 아래로 스크롤한 뒤 **Delete Account** 탭.
3. 다이얼로그에서 확인.

![Edit Profile의 Delete Account](../images/delete-account.png)

이렇게 하면 계정과 프로필이 영구적으로 삭제됩니다. 본인 소유가 아닌 대회 데이터는 삭제되지 않습니다 — 예를 들어 선수로 참가했다면, organizer의 시작 명단과 결과에 남은 본인 엔트리는 해당 이벤트 기록의 일부로 유지됩니다.
