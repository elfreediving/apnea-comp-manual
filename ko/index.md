...
13. [일정 조정 (OT Delay)](#일정-조정-ot-delay)
14. [푸시 알림](#푸시-알림)
15. [오프라인 동작](#오프라인-동작)
16. [AIDA 연동](#aida-연동)
17. [멀티데이 이벤트](#멀티데이-이벤트)
18. [결과 및 내보내기](#결과-및-내보내기)
19. [FAQ](#faq)
20. [트러블슈팅](#트러블슈팅)
21. [개인정보 및 데이터](#개인정보-및-데이터)
22. [연락처](#연락처)
...
1. 앱을 열고 로그인.  
   *팁: 로그인 화면의 **Remember email**을 체크하면 다음에 이메일이 자동으로 채워집니다. 비밀번호는 디바이스에 저장되지 않습니다.*
...
**직접 라인 지정 (Direct lane assignment)** — 특수한 경우(예: 국가 기록 시도용 특정 레인 예약)에는 선수를 탭하고 목표 레인을 직접 선택하세요. 일정이 재구성됩니다.
...
OT 이전 정해진 시점에 음성 안내가 나갑니다:
`5:00`, `4:00`, `3:00`, `2:00`, `1:30`, `1:00`, `0:30`, `0:20`, `0:10`, 그 후 `0:10`부터 `0`까지 매초, 마지막으로 "Official top!"

OT 기준 −25초 ~ −30초 구간에서는 "thirty"가 빠르게 안내됩니다 (뒤따르는 "twenty"와 겹치지 않도록). −31초에는 출발하지 않은 선수에 대해 "Start cancelled"가 정상 속도로 안내됩니다. 그 외 안내는 표준 속도입니다.
...
- **저장하는 것** — 계정 이메일, 표시 이름, 프로필 사진 (업로드한 경우), AIDA 선수 UUID (선수로 가입한 경우), 푸시 알림 구독 ID (알림 활성화한 경우), 입력한 대회 데이터 (선수 결과, 체크인 상태, 활동 로그).
...
- **기기 로컬 저장소** — 로그인 시 **Remember email**을 체크하면 이메일 주소가 기기 로컬 저장소(Android의 SharedPreferences, iOS의 NSUserDefaults)에 저장되어 다음 로그인 시 자동 입력됩니다. 비밀번호는 절대 저장되지 않습니다. Remember email 체크를 해제하거나 앱을 삭제하면 제거됩니다.
