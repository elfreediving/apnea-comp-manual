...
> 사인 그림은 시각적 확인용으로만 사용 — 체크인 순간 스태프에게 보여주고 시트가 닫히는 즉시 폐기됩니다. 사인 이미지는 어디에도 저장되지 않습니다 (서버, 디바이스 모두).

> 체크인도 오프라인에서 동작합니다. 오프라인 상태로 확정해도 디바이스 메모리에 저장되며 카드에 작은 ⛔ "Offline" 배지가 표시됩니다. 인터넷 복구 후 5초 안에 자동 동기화됩니다. [오프라인 운영](#오프라인-운영) 참조.
...
### 체크인 도중 인터넷이 끊겼어요. 어떻게 되나요?

체크인이 디바이스에 로컬 저장되고 선수 카드에 작은 ⛔ "Offline" 배지가 표시됩니다. 인터넷 복구 후 5초 안에 자동으로 서버와 동기화됩니다. 선수가 다시 사인할 필요 없어요. 사인 그림 자체는 저장되지 않으며, 그 순간 시각적 확인용으로만 사용됩니다.
